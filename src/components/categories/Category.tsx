...
const Category: React.FC<CategoryProps> = ({ category }: { category: string }) => {
    const history = useHistory();
    const [posts, setPosts] = useState([])
    const { myPosts, setMyPosts, addLike, getAllPosts, myInfo, updateUser, getUserPosts, addDislike } = useContext(MyContext);
    ...
    const getPosts = async () => {
        try {
            const result = await fetch(`http://localhost:3000/api/getPostCategory?category=${category}`, {
                method: "GET",
                headers: {
